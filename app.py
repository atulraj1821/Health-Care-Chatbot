...
def login_validation():
  ...
  if len(users)>0:
      #session['user_id']=users[0][0]#session got settled
...
def add_user():
    ...
    cursor.execute("""INSERT  INTO `login` (`name`,`email`,`password`)VALUES('{}','{}','{}')""".format(name,email,password))
    conn.commit()
    cursor.execute("""SELECT * FROM `login` where `email` LIKE '{}'""".format(email))
    myuser=cursor.fetchall()
    #session['user_id']=myuser[0][0]## session set
    return redirect('/home')
